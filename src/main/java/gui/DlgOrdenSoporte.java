...
public class DlgOrdenSoporte extends JDialog implements ActionListener {
	...
	void listar() {
		EntityManager manager = JPAUtil.getEntityManager();
		String jpql = "select o from Orden_Soporte o";
		
		try {
			List<Orden_Soporte> lstOrdenes = manager.createQuery(jpql,Orden_Soporte.class).getResultList();
			
			for(Orden_Soporte orden_Soporte:lstOrdenes){
				Tecnico tecnico = orden_Soporte.getIdTecnico();
				Cliente cliente = orden_Soporte.getIdCliente();
				
				imprimir("Nro de Orden....:"+orden_Soporte.getNroOrden());
				imprimir("Fecha Registro....."+orden_Soporte.getFechaRegistro());
				imprimir("Tecnico....."+tecnico.getNombre()+ " especialidad en " + tecnico.getespecialidadDescripcion());
				imprimir("Cliente......"+cliente.getRuc()+"-"+cliente.getRazonSocial());
				imprimir("Monto........."+"S/."+orden_Soporte.getMonto());
				imprimir("Detalle Incidencia........"+orden_Soporte.getDetalleIncidencia());
				imprimir("--------------------------\n");
			}
	...
	void consultar() {
		...
		try {
			Orden_Soporte orden_Soporte = manager.find(Orden_Soporte.class, nroOrden);
			if (orden_Soporte == null) {
				mensajeAdvertencia("Orden de soporte no encontrada");
				return;
			}
			txtDetalleIncidencia.setText(orden_Soporte.getDetalleIncidencia());
			cboTecnicos.setSelectedItem(orden_Soporte.getIdTecnico());
			cboClientes.setSelectedItem(orden_Soporte.getIdCliente());
			txtMonto.setText(orden_Soporte.getMonto()+"");
			txtFechaRegistro.setText(orden_Soporte.getFechaRegistro()+"");
	...
	void modificar() {
		...
		try {
			...
			manager.getTransaction().begin();
			manager.merge(ordeSoporte);
			manager.getTransaction().commit();
			
			mensajeInfo("Orden de soporte actualizado");
			limpiar();
		} catch (Exception e) {
			mensajeError("Hubo un error en la transacion");
			e.printStackTrace();
